...
function TurtleDrawing(canvas, gridButton, helpButton) {
    this.points = [];
    this.canvas = canvas;
...
function render({ model, el }) {
  ...
  let turtleArea = document.createElement("div");
  turtleArea.id = "turtle-canvas-area";
  toinsert.append(turtleArea);
  let buttonDiv = document.createElement("div");
  buttonDiv.target = "button-area";
  let helpButton = document.createElement("button");
  helpButton.append("Help!");
  buttonDiv.append(helpButton);
  let hButton = document.createElement("button");
  hButton.append("test");
  buttonDiv.append(hButton);
  let gridButton = document.createElement("button");
  gridButton.id = "grid-element";
  ...
  canvas.height = 401;
  canvas.resize;
  canvas.style.background = "#99CCFF";
  canvasDiv.append(canvas);
  var turtledrawing = new TurtleDrawing(canvas, gridButton, helpButton);
  
  //This doesn't actually get the values. model.get('points') works but I don't know how to store the points 
  turtledrawing.points = model.get('points');
  
  hButton.addEventListener("click", () => {
      alert(model.get('points')[0]['p']);
      alert(turtledrawing.points);
    });
  el.appendChild(toinsert);
  window.debugturtle = el;
}
